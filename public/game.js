(() => {
  "use strict";
  const Runner = function (outerContainerId, opt_config) {
  ...
  Runner.prototype = {
     ...
     */
    init: function () {
      // Hide the static icon if it exists.
      const iconElement = document.querySelector("." + Runner.classes.ICON);
      if (iconElement) {
      ...
      // Check if the container element exists by its ID.
      const existingContainer = document.getElementById(
        Runner.classes.CONTAINER,
      );
      if (existingContainer) {
        existingContainer.innerHTML = ""; // Clear the existing content
        this.containerEl = existingContainer; // Use the existing container
        this.dimensions = Runner.defaultDimensions;
      } else {
        this.containerEl = document.createElement("div");
    ...
    handleEvent: function (e) {
      return function (evtType, events) {
        switch (evtType) {
          case events.KEYDOWN:
            if (e.code === "Space" && !this.config.onGameDisabled()) {
              this.onKeyDown(e);
            }
            break;
          case events.KEYUP:
            if (e.code === "Space" && !this.config.onGameDisabled()) {
              this.onKeyUp(e);
            } else if (e.target?.nodeName != "INPUT" && !e.target?.form) {
    ...
    setArcadeMode() {
      document
        .getElementById(Runner.classes.GAME_WRAPPER)
        .classList.add(Runner.classes.ARCADE_MODE);
